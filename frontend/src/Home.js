...
function Home() {
  ...
  // FETCH NEW VEHICLES FROM DATABASE
  useEffect(() => {
  ...
  const getNavLinkStyle = (path) => ({
    background: 'none',
    border: 'none',
    borderBottom: location.pathname === path ? `2px solid ${accentColor}` : 'none',
    color: location.pathname === path ? accentColor : 'white',
    cursor: 'pointer',
    fontWeight: 'bold',
    fontSize: '1rem',
    paddingBottom: '5px',
  ...
  const staticVehicles = [
    ...
    { id: 8, name: "Suzuki Every", type: "Van", price: 8000, fuel: "Petrol", location: "Kurunegala", kmLimit: "100 KM", img: "https://www.ccarprice.com/products/Suzuki_Every_2025.jpg" },
    ...
    { id: 16, name: "Toyota Noah", type: "Car", price: 24000, fuel: "Hybrid", location: "Kandy", kmLimit: "200 KM", img: "https://global.toyota/pages/news/images/2022/01/13/1330/003.jpg" },
  ...
  // COMBINE STATIC LIST WITH DATABASE LIST
  const allVehicles = [...dbVehicles, ...staticVehicles];
  ...
  const handleBookingSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setBookingDate(""); setBookingTime(""); setPhone("");
      } else {
    ...
    <div style={{ backgroundColor: '#071618', minHeight: '100vh', color: 'white', fontFamily: 'Poppins, sans-serif' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          ...
          <div style={{ display: 'flex', gap: '20px', marginLeft: '30px' }}>
            <button onClick={() => navigate("/home")} style={getNavLinkStyle("/home")}>Home</button>
            <button onClick={() => navigate("/my-bookings")} style={getNavLinkStyle("/my-bookings")}>My Bookings</button>
            {userName === "mnmrukshan22" && (
              <button onClick={() => navigate("/admin")} style={getNavLinkStyle("/admin")}>Admin Panel</button>
            )}
          </div>
        </div>
        ...
        <div style={{ flex: 1, maxWidth: '400px', margin: '0 30px', position: 'relative', display: 'flex', alignItems: 'center' }}>
          <input
            type="text" placeholder="Search your favorite ride..." value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            style={{ width: '100%', padding: '10px 45px 10px 20px', borderRadius: '25px', border: '1px solid rgba(255,255,255,0.2)', background: 'rgba(255,255,255,0.05)', color: 'white', outline: 'none' }}
          />
          {searchTerm && <button onClick={() => setSearchTerm("")} style={{ position: 'absolute', right: '15px', background: 'transparent', border: 'none', color: 'rgba(255,255,255,0.5)', fontSize: '18px', cursor: 'pointer' }}>✕</button>}
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', background: 'rgba(255, 255, 255, 0.05)', padding: '5px 15px 5px 8px', borderRadius: '20px', border: '1px solid rgba(255, 255, 255, 0.1)' }}>
            <div style={{ width: '32px', height: '32px', borderRadius: '50%', backgroundColor: accentColor, display: 'flex', justifyContent: 'center', alignItems: 'center', color: '#071618', fontWeight: 'bold', fontSize: '14px' }}>{userName.charAt(0).toUpperCase()}</div>
            <span style={{ fontSize: '0.9rem', color: 'rgba(255,255,255,0.7)' }}>Welcome, <strong style={{color: accentColor, fontSize: '1rem'}}>{userName}</strong></span>
          </div>
      ...
      <div style={{ textAlign: 'center', padding: '60px 20px' }}>
        ...
        <div style={{ margin: '30px auto', maxWidth: '300px' }}>
          ...
          <input type="range" min="3000" max="30000" step="500" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} style={{ width: '100%', accentColor: accentColor }} />
        </div>
        <div style={{ marginTop: '40px', display: 'flex', justifyContent: 'center', gap: '20px', flexWrap: 'wrap' }}>
          {["All", "Car", "Bike", "Van"].map(cat => (
            <button key={cat} onClick={() => setCategory(cat)} style={{ padding: '12px 40px', borderRadius: '50px', border: '1px solid rgba(255,255,255,0.1)', backgroundColor: category === cat ? accentColor : 'rgba(255, 255, 255, 0.05)', color: category === cat ? '#071618' : 'white', cursor: 'pointer', fontWeight: 'bold', backdropFilter: 'blur(10px)', transition: 'all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275)', transform: category === cat ? 'scale(1.15)' : 'scale(1)', boxShadow: category === cat ? `0 0 30px ${accentColor}88` : 'none' }}>{cat === "All" ? "All Rides" : cat + "s"}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '40px', padding: '0 60px 100px' }}>
        {filteredVehicles.map(vehicle => (
          <div key={vehicle.id || vehicle._id} className="vehicle-card" style={{ background: 'rgba(255, 255, 255, 0.02)', borderRadius: '30px', overflow: 'hidden', border: '1px solid rgba(255, 255, 255, 0.08)', backdropFilter: 'blur(20px)', transition: 'all 0.4s ease', cursor: 'pointer' }}
          ...
          >
            <div style={{ position: 'relative' }}>
              ...
              <div style={{ position: 'absolute', top: '15px', left: '15px', background: 'rgba(0, 209, 178, 0.15)', backdropFilter: 'blur(12px)', padding: '6px 14px', borderRadius: '12px', border: '1px solid rgba(0, 209, 178, 0.4)', fontSize: '0.75rem', fontWeight: 'bold', color: '#00d1b2', boxShadow: '0 4px 15px rgba(0, 0, 0, 0.4)', zIndex: 5 }}>
                ⚡ {vehicle.kmLimit || "100 KM"}
              </div>
            </div>
            <div style={{ padding: '30px' }}>
              <h3 style={{ margin: '0 0 10px 0', fontSize: '1.5rem' }}>{vehicle.name}</h3>
              {/* 👈 FINAL UPDATED TRANSMISSION LOGIC */}
              <p style={{ color: 'rgba(255,255,255,0.5)' }}>
                
                 ⛽ {vehicle.fuel || "Petrol"} • ⚙️ {(vehicle.name.includes("PCX") || vehicle.name.includes("Dio") || vehicle.name.includes("RayZR") || vehicle.type !== "Bike") ? "Auto" : "Manual"}
              </p>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '20px' }}>
                <span style={{ fontSize: '1.6rem', fontWeight: '900', color: accentColor }}>LKR {vehicle.price}</span>
                <button onClick={() => setSelectedVehicle(vehicle)} className="book-btn-professional">BOOK NOW</button>
              </div>
            </div>
          </div>
        ))}
      ...
      {selectedVehicle && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(15px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 2000, animation: 'fadeIn 0.3s ease-out' }}>
          <div style={{ background: 'rgba(255, 255, 255, 0.03)', padding: '45px', borderRadius: '35px', border: '1px solid rgba(255, 255, 255, 0.12)', width: '90%', maxWidth: '460px', textAlign: 'center', boxShadow: '0 30px 70px rgba(0,0,0,0.7)', animation: 'slideUp 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)' }}>
            ...
            <form onSubmit={handleBookingSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '22px' }}>
              <div style={{ textAlign: 'left' }}><label style={{ fontSize: '0.8rem', opacity: 0.7, marginBottom: '8px', display: 'block', color: accentColor, fontWeight: 'bold' }}>Pickup Date</label><input type="date" required value={bookingDate} onChange={(e) => setBookingDate(e.target.value)} style={{ width: '100%', padding: '16px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', color: 'white', outline: 'none', transition: '0.3s', colorScheme: 'dark', accentColor: accentColor }} /></div>
              <div style={{ textAlign: 'left' }}><label style={{ fontSize: '0.8rem', opacity: 0.7, marginBottom: '8px', display: 'block', color: accentColor, fontWeight: 'bold' }}>Pickup Time</label><input type="time" required value={bookingTime} onChange={(e) => setBookingTime(e.target.value)} style={{ width: '100%', padding: '16px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', color: 'white', outline: 'none', transition: '0.3s', colorScheme: 'dark', accentColor: accentColor }} /></div>
              <div style={{ textAlign: 'left' }}><label style={{ fontSize: '0.8rem', opacity: 0.7, marginBottom: '8px', display: 'block', color: accentColor, fontWeight: 'bold' }}>Phone Number</label><input type="text" required placeholder="07XXXXXXXX" value={phone} onChange={(e) => setPhone(e.target.value)} style={{ width: '100%', padding: '16px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', color: 'white', outline: 'none' }} /></div>
              <div style={{ display: 'flex', gap: '20px', marginTop: '15px' }}>
                <button type="button" onClick={() => setSelectedVehicle(null)} style={{ flex: 1, padding: '15px', background: 'rgba(255, 255, 255, 0.05)', border: '1px solid rgba(255, 255, 255, 0.1)', color: 'white', borderRadius: '18px', cursor: 'pointer', transition: '0.2s', fontWeight: 'bold' }}>Cancel</button>
                <button type="submit" style={{ flex: 2, padding: '15px', background: accentColor, border: 'none', color: '#071618', fontWeight: '900', borderRadius: '18px', cursor: 'pointer', transition: '0.2s', boxShadow: `0 10px 25px ${accentColor}44` }}>Confirm</button>
              </div>
            </form>
          </div>
      ...
      {/* 🛠️ CSS FOR PROFESSIONAL LIGHTING & ANIMATION 🛠️ */}
      <style>{`
        .book-btn-professional {
        ...
        .book-btn-professional:active {
          transform: scale(0.95);
        }

        .book-btn-professional::after {
          content: "";
          position: absolute;
          top: -50%;
          left: -60%;
        ...
        .book-btn-professional:hover::after {
          left: 120%;
        }
      `}</style>
      <Footer />
